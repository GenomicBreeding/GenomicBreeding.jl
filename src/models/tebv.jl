...
mutable struct TEBV
    ...
    function TEBV(;
        ...
        return new(traits, formulae, models, df_BLUEs, df_BLUPs, df_TEBVs)
    end
end
...
Hash a TEBV struct using the traits, formualae and df_TEBVs.
...
function Base.hash(x::TEBV, h::UInt)::UInt
    # hash(TEBV, hash(x.traits, hash(x.formulae, hash(x.models, hash(x.df_BLUEs, hash(x.df_BLUPs, hash(x.df_TEBVs, h)))))))
    hash(TEBV, hash(x.traits, hash(x.formulae, hash(x.df_TEBVs, h))))
end
...
function checkdims(tebv::TEBV)::Bool
    ...
    if !isa(tebv.traits, Vector{String}) ||
       ...
       !isa(tebv.df_TEBVs, Vector{DataFrame})
        return false
    end
    return true
end
...
function analyse(trials::Trials; max_levels::Int64 = 100, max_time_per_model::Int64 = 60, verbose::Bool = true)::TEBV
    ...
    for trait in unique(trials.traits)
        ...
        # Extract the random or fixed effects of the entries
        df_TEBVs::DataFrame = DataFrame(entries = entries, tebv = fill(Inf64, length(entries)))
        if length(idx_col) > 1
            df_TEBVs[!, "entries"] = df_BLUPs[:, 1]
            df_TEBVs[!, "tebv"] = sum.(eachrow(df_BLUPs[:, idx_col[2:end]]))
        end
        if length(idx_row) == (n - 1)
            df_TEBVs[!, "entries"] = df_BLUEs[!, "Name"]
            df_TEBVs[!, "tebv"] = df_BLUEs[!, "Coef."]
        end
