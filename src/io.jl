...
function loadgenomesphenomes(input::GBInput)::Tuple{Genomes,Phenomes,Vector{String},Vector{String}}
    ...
    # Load the phenomic dataf for GP CV, fitting, single-step fit-predict (because we are fitting first before predicting GEBVs) and GWAS
    phenomes = if (fname_pheno != "") && (input.analysis ∈ [cv, fit, fitandpredict, gwas])
        try
        ...
        # For predicting the GEBVs after models have been fitted
...
function prepareinputs(input::GBInput)::Vector{GBInput}
    ...
    elseif input.analysis ∈ [gwas]
        ...
        throw(ArgumentError("Analysis: `" * string(input.analysis) * "` invalid."))
    end
...
"""
    loadcvs(input::GBInput; min_train_size::Int64=10, verbose::Bool=true)::Vector{CV}
...
- `verbose::Bool=true`: Whether to display a progress meter during loading (default: true)
...
A progress meter is displayed during loading when verbose is true.
...
```
"""
function loadcvs(input::GBInput; min_train_size::Int64 = 10, verbose::Bool = true)::Vector{CV}
