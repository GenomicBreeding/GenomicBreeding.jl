...
function simulatemating(;
    ...
    loci_fin_idx::Array{Int64,1} = loci(parent_genomes)
    ...
    for t = 1:n_generations
        ...
        for chr in unique_chromosomes
            ...
            idx_loci_alleles::Array{Int64,1} = findall(chromosomes_per_locus_allele .== chr)
            idx_loci::Array{Int64,1} = findall(chromosomes_per_locus .== chr)
            allele_freqs::Array{Float64,2} =
                parent_genomes.allele_frequencies[:, idx_loci_alleles]
            μ::Array{Float64,1} = mean(allele_freqs, dims = 1)[1, :]
            Σ::Array{Float64,2} = StatsBase.cov(allele_freqs)
            max_iter::Int64 = 10
            iter::Int64 = 1
            ...
            progeny_allele_freqs::Array{Float64,2} =
            ...
            for j in eachindex(loci_ini_idx[idx_loci])
                ...
                a = idx_fin - idx_ini
                sum_of_prev_allele_freqs::Array{Float64,1} =
